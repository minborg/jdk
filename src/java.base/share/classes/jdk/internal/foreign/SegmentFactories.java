...
public class SegmentFactories {
    ...
    /**
     * {@return a new MemorySegment that can be freed using {@linkplain #free(MemorySegment)}}
     * <p>
     * The segment's {@linkplain MemorySegment#maxByteAlignment()} >= 8.
     *
     * @param byteSize to allocate
     */
    public static MemorySegment makeNativeSegmentUnchecked(long byteSize) {
        return makeNativeSegmentUnchecked(allocateMemoryWrapper(byteSize), byteSize);
    }

    public static void free(MemorySegment segment) {
        if (segment.scope() != MemorySessionImpl.GLOBAL_SESSION) {
