 ...
 * @enablePreview
 * @run junit/othervm --add-opens java.base/java.util=ALL-UNNAMED UnboundLazyListTest
 */
...
final class UnboundLazyListTest {
    ...
    void factoryInvariants() {
        assertThrows(NullPointerException.class, () -> List.ofLazy(null));
    }
    ...
    void add(BiConsumer<List<Integer>, Integer> target) {
        var lazy = newEmptyLazyList();
        target.accept(lazy, 0);
        assertEquals(0, lazy.get(0));
        assertEquals(1, lazy.size());
        target.accept(lazy, 1);
        assertEquals(1, lazy.get(1));
    ...
    static <T extends Throwable> void assertThrowsForOperation(Class<T> expectedType, Operation operation) {
        ...
        assertThrows(expectedType, () -> operation.accept(subSub), subSub.getClass().getName());
    }
    ...
    record ListFunction(String name,
        ...
        @Override
        public Object apply(List<Integer> list) {
    ...
    static List<Integer> newEmptyLazyList() {
        return List.ofLazy(Integer.class);
    }
