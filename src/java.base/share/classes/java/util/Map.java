...
public interface Map<K, V> {
    ...
    /**
     * {@return a builder for stable maps}
     * <p>
     ...
     *     Map<String, Integer> map = Map.ofStable(String::length).toMap();
     *     // Creates a bound map with at most 10 associations
     *     // (with arbitrary keys) and where threads may race to make associations.
     *     Map<String, Integer> map = Map.ofStable(String::length).withInitialMappingSize(10).toMap();
     *
     ...
     *     // per key.
     *     Map<String, Integer> map = Map.ofStable(String::length)
     ...
     * @param <V>               the type of mapped values in the returned map
     */
    static <K, V> StableBuilder<K, V> ofStable() {
        return new LazyCollections.StableBuilderImpl<>();
    }
     ...
     * @param <V>               the type of mapped values in the returned map
     */
    sealed interface StableBuilder<K, V> permits LazyCollections.StableBuilderImpl {
         ...
         */
        StableBuilder<K, V> withKeys(Set<? extends K> keys);

        /**
         * Provide an {@code initialMappingCapacity} to the builder.
         * <p>
         * This method would from now imply a <em>bound</em> map where only {@code size}
         * associations can be made.
         *
         * @param initialMappingCapacity to use
         * @return a new builder with the provided {@code size}
         * @throws IllegalArgumentException if the provided {@code initialMappingCapacity}
         *         is negative.
         */
        StableBuilder<K, V> withInitialMappingCapacity(int initialMappingCapacity);

        /**
         * Provide a synchronized property to the builder.
         * <p>
         * This method would from now imply a <em>synchronized</em> builder where
         * only one competing thread will compute values.
         *
         * @return a new builder with the synchronized property set
         */
        StableBuilder<K, V> withSynchronization();

        // LazyBuilder<K, V> withKeyType(Class<T> keyType);
