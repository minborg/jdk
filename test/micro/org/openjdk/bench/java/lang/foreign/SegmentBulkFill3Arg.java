...
@Fork(value = 3)
public class SegmentBulkFill3Arg {
    ...
    @Setup
    public void setup() {
        array = new byte[ELEM_SIZE + OFFSET];
        heapSegment = MemorySegment.ofArray(array);
        nativeSegment = Arena.ofAuto().allocate(ELEM_SIZE + OFFSET, 8);
        unalignedSegment = Arena.ofAuto().allocate(ELEM_SIZE + 1 + OFFSET, 8).asSlice(1);
        buffer = ByteBuffer.wrap(array);
    }

    @Benchmark
    public void heapSegmentFillJava() {
        heapSegment.fill(OFFSET, ELEM_SIZE, (byte) 0);
    }

    @Benchmark
    public void heapSegmentFillJavaSlice() {
        heapSegment.asSlice(OFFSET, ELEM_SIZE).fill((byte) 0);
    }
